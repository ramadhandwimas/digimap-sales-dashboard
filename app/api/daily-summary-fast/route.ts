...
type Row={date:string;invoice:string;description:string;qty:number;amount:number;category:string;brand:string;scheme:string;vendor:string;article:string;store:string};
function parse(r:unknown[]):Row{return{date:iso(r[0]),invoice:s(r[3]),article:s(r[4]),description:s(r[5]),qty:n(r[7]),amount:n(r[8]),category:up(r[9]),brand:up(r[10]),scheme:up(r[12]),vendor:up(r[13]),store:up(r[15])}}
function valid(r:Row){return!!r.date&&r.store==="M238"&&r.scheme!=="VOUCHER"&&!up(`${r.article} ${r.description}`).includes("VOUCHER")}
function kind(r:Row){const text=`${up(r.category)} ${up(r.description)}`;if(/HALO\s*IPHONE|HALOIPHONE|\bHALO\b/.test(text))return"vas";if(r.scheme==="VAS")return"vas";if(r.scheme==="ACCESSORIES")return"accessories";if(r.scheme==="DEVICES")return"device";return"other"}
function product(r:Row){if(kind(r)!=="device"||r.brand!=="APPLE")return"";if(r.scheme==="ACCESSORIES"&&r.category==="AIRPODS")return"airpods";if(r.category==="IPHONE")return"iphone";if(r.category==="MAC"||r.category==="MACBOOK")return"mac";if(r.category==="IPAD")return"ipad";if(r.category==="APPLE WATCH"||r.category==="WATCH")return"watch";return""}
...
export async function GET(req:NextRequest){
 const started=Date.now(),apiStarted=getGoogleSheetRequestCount(),email=process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL,key=process.env.GOOGLE_PRIVATE_KEY,from=req.nextUrl.searchParams.get("from")||"",to=req.nextUrl.searchParams.get("to")||"",viewMode=req.nextUrl.searchParams.get("mode")||"range";
