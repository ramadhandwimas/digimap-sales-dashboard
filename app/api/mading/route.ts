...
function vas(r:Row){const t=`${r.article} ${r.brand} ${r.vendor} ${r.description}`.toUpperCase();if(t.includes("QOALA")||t.includes("KLA"))return"Qoala";if(t.includes("TELKOMSEL")||t.includes("TSL"))return"Telkomsel";if(t.includes("INDOSAT")||t.includes("IDT"))return"Indosat";if(/(^|\s)XL(\s|$)|XXL/.test(t))return"XL";return""}
function summarize(rows:Row[]){
 ...
 const lobs=new Map(["iPhone","MacBook","iPad","Apple Watch","AirPods"].map(x=>[x,{name:x,qty:0,value:0}]));
 const providers=new Map(["Qoala","Telkomsel","XL","Indosat"].map(x=>[x,{name:x,qty:0,value:0}]));
 for(const r of rows.filter(valid)){total.amount+=r.amount;total.qty+=r.qty;if(r.invoice)invoices.add(r.invoice);const k=bucket(r);if(k==="device")total.device+=r.amount;if(k==="accessories")total.accessories+=r.amount;if(k==="vas")total.vas+=r.amount;staff.set(r.name,(staff.get(r.name)||0)+r.amount);const l=lob(r);if(l){const x=lobs.get(l)!;x.qty+=r.qty;x.value+=r.amount}const v=vas(r);if(v){const x=providers.get(v)!;x.qty+=r.qty;x.value+=r.amount}}
 total.invoices=invoices.size;return{...total,upt:total.invoices?total.qty/total.invoices:0,staff:[...staff.entries()].map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value),lob:[...lobs.values()],vas:[...providers.values()]};
}
...
function cutoffFor(period:string,asOf:string,minusOne=false){const days=dim(period),requested=Math.max(1,Math.min(days,Number(asOf.slice(8,10))-(minusOne?1:0)));return`${period}-${String(requested).padStart(2,"0")}`}
...
export async function GET(req:NextRequest){
 ...
 try{
  const period=req.nextUrl.searchParams.get("period")||today().slice(0,7),asOf=req.nextUrl.searchParams.get("asOf")||today();const year=period.slice(0,4),source=year==="2025"?"Data Copas Archive 2025":"Data Copas";
