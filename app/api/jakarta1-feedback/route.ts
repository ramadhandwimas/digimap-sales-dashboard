...
const LABELS:Record<string,string>={external:"Faktor External",internal:"Internal",bnpl:"Promo BNPL",stock:"Ketersediaan Stok"};
...
function professional(raw:string,category:string,store:string){const clean=raw.replace(/\s+/g," ").trim().replace(/[.!]+$/g,"");const lead=LABELS[category]??"Faktor Penjualan";return `${lead}: berdasarkan evaluasi ${store}, ${clean.charAt(0).toLowerCase()+clean.slice(1)}. Kondisi ini menjadi perhatian utama untuk tindak lanjut dan perbaikan pencapaian harian.`}
