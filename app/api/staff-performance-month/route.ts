...
function kind(r:unknown[]){const scheme=up(r[12]),text=`${up(r[6])} ${up(r[9])} ${up(r[5])}`;if(/HALO\s*IPHONE|HALOIPHONE|\bHALO\b/.test(text))return"vas";if(scheme==="VAS")return"vas";if(scheme==="ACCESSORIES")return"accessories";if(scheme==="DEVICES")return"device";if(/IPHONE|IPAD|MAC|APPLE WATCH|AIRPODS/.test(text))return"device";return"other"}
function product(r:unknown[]){if(kind(r)!=="device"||up(r[10])!=="APPLE")return"other";const scheme=up(r[12]),cat=up(r[9]);if(scheme==="ACCESSORIES")return cat==="AIRPODS"?"airpods":"other";if(cat==="IPHONE")return"iphone";if(cat==="MAC")return"mac";if(cat==="IPAD")return"ipad";if(cat==="APPLE WATCH")return"watch";return"other"}
function vasType(r:unknown[]){const t=`${s(r[4])} ${s(r[10])} ${s(r[13])} ${s(r[6])} ${s(r[9])} ${s(r[5])}`.toUpperCase();if(t.includes("QOALA")||t.includes("KLA"))return"qoala";if(t.includes("TELKOMSEL")||t.includes("TSL")||t.includes("HALO IPHONE")||t.includes("HALOIPHONE")||/\bHALO\b/.test(t))return"telkomsel";if(t.includes("INDOSAT")||t.includes("IDT"))return"indosat";if(/(^|\s)XL(\s|$)|XXL/.test(t))return"xl";return""}
