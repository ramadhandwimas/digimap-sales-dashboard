...
export async function GET(req:NextRequest){
  const started=Date.now(),apiStart=getGoogleSheetRequestCount();
  ...
  try{
    ...
    const apiRequests=getGoogleSheetRequestCount()-apiStart;
    console.warn("M238_PERF",{op:"read-soh",total:Date.now()-started,apiRequests,error:err instanceof Error?err.message:String(err)});
    return NextResponse.json({error:err instanceof Error?err.message:"Gagal membaca SOH"},{status:500});
  }
}
