...
export async function GET(req:NextRequest){
  ...
  try{
    ...
    const lobList=(['iPhone','Mac','iPad','Apple Watch'] as Lob[]);
    const lobs=lobList.map(lob=>{const sum=(year:string)=>STORES.reduce((a,s)=>{a.amount+=years[year][s].lobs[lob].amount;a.qty+=years[year][s].lobs[lob].qty;return a},empty());return{lob,...compare(sum("2025"),sum("2026"))}}).sort((a,b)=>a.gap-b.gap);
    ...
    return NextResponse.json({endMonth,summary:{...compare(a25,a26),growthStores:stores.filter(s=>(s.yoy??0)>=0).length,disgrowthStores:stores.filter(s=>(s.yoy??0)<0).length},stores,months,lobs,detail,meta:{scope:"Apple Device only",includedLobs:["iPhone","Mac","iPad","Apple Watch"],excluded:["Accessories","VAS","AirPods","Voucher"]}},{headers:{"Cache-Control":"private, max-age=60"}});
