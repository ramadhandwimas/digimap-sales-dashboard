...
export async function GET(req:NextRequest){
 ...
 try{
  const dataResult=await safeRanges(DASHBOARD_ID,["'Data Copas'!A2:S50000"],email,key),rows=dataResult[0]||[];
  const [stockResult,trafficResult]=await Promise.all([
   safeRanges(DASHBOARD_ID,["'SOH'!C10:E200","'SOH'!J10:L200","'SOH'!Q10:S200","'SOH'!X10:Z200","'SOH'!AE10:AG200"],email,key),
   safeRanges(MASTER_ID,["'Traffic'!A2:B1000"],email,key),
  ]),trafficRows=trafficResult[0]||[];
  const [iphone=[],ipad=[],mac=[],watch=[],airpods=[]]=stockResult;
  ...
  const a=makeSide(from),b=makeSide(to),stock=new Map<string,number>();
  for(const group of [iphone,ipad,mac,watch,airpods])for(const r of group){const article=s(r[0]).toUpperCase(),q=n(r[2]);if(article&&article!=="ARTICLE"&&article!=="GRAND TOTAL")stock.set(article,q)}
