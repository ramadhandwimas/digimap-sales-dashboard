...
async function build(req:NextRequest,email:string,key:string,force:boolean):Promise<DailySummaryPayload>{
 ...
 const config=await getConfig(email,key,force),current=await readRows(startDate,endDate,email,key,force),dailyRows=aggregate(current.rows,config),target=mode==="monthly"?monthlyTarget(config,period):dailyRows.reduce((a,d)=>a+d.target,0),tot=summarize(dailyRows,target)
 ...
 const warnings=["Traffic/CVR source belum ditemukan pada source terstruktur yang diaudit; field dipertahankan sebagai null agar tidak mengarang angka."]
