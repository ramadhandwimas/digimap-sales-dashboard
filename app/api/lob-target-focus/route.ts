...
function activeFocusFromConfig(rows:unknown[][],weeks:Set<string>){
 const weekLocs:Array<{r:number;c:number;week:string}>=[],productLocs:Array<{r:number;c:number;name:string}>=[];
 for(let r=0;r<rows.length;r++)for(let c=0;c<(rows[r]?.length||0);c++){const wk=weekKey(rows[r][c]);if(wk)weekLocs.push({r,c,week:wk});const p=focusCell(rows[r][c]);if(p)productLocs.push({r,c,name:p})}
 const wanted=weekLocs.filter(x=>weeks.has(x.week)),hits:string[]=[];
 const push=(name:string)=>{if(!hits.includes(name))hits.push(name)};
 for(const p of productLocs){if(wanted.some(w=>(w.r===p.r&&Math.abs(w.c-p.c)<=16)||(w.c===p.c&&Math.abs(w.r-p.r)<=18)||(Math.abs(w.r-p.r)+Math.abs(w.c-p.c)<=10)))push(p.name)}
 if(hits.length)return hits;
 const scoped:string[]=[];for(let r=30;r<rows.length;r++)for(let c=16;c<(rows[r]?.length||0);c++){const p=focusCell(rows[r][c]);if(p&&!scoped.includes(p))scoped.push(p)}
 return scoped;
}
...
export async function GET(req:NextRequest){const email=process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL,key=process.env.GOOGLE_PRIVATE_KEY;if(!email||!key)return NextResponse.json({error:"Google Sheets belum dikonfigurasi"},{status:503});try{
 const[dataRows,configRows]=await getSheetRanges(ID,["'Data Copas'!A2:S50000","Config!A1:AZ120"],email,key);
 const all=dataRows.filter(r=>s(r[15]).toUpperCase()===STORE&&s(r[18])==="2026"),roster=configRows.slice(27,55).filter(r=>s(r[7])===STORE&&s(r[8])&&s(r[9])&&!/SUPERVISOR|ONLINE/i.test(s(r[10]))).map(r=>({id:s(r[8]),name:s(r[9])})),byId=new Map(roster.map(r=>[r.id,r])),byName=new Map(roster.map(r=>[up(r.name),r]));
 const availableWeeks=[...new Set(all.map(r=>weekKey(r[14])).filter(Boolean))].sort((a,b)=>weekOrder(a)-weekOrder(b)),availableMonths=[...new Set(all.map(r=>iso(r[0]).slice(0,7)).filter(v=>/^2026-\d{2}$/.test(v)))].sort();
 const mode=(req.nextUrl.searchParams.get("mode")||"week") as "week"|"month"|"range",requestedWeek=weekKey(req.nextUrl.searchParams.get("week")||""),week=availableWeeks.includes(requestedWeek)?requestedWeek:(availableWeeks.at(-1)||""),requestedMonth=req.nextUrl.searchParams.get("month")||availableMonths.at(-1)||"2026-09",month=availableMonths.includes(requestedMonth)?requestedMonth:(availableMonths.at(-1)||requestedMonth),from=req.nextUrl.searchParams.get("from")||`${month}-01`,to=req.nextUrl.searchParams.get("to")||`${month}-31`,selected=all.filter(r=>{if(mode==="week")return weekKey(r[14])===week;const d=iso(r[0]);return mode==="month"?d.startsWith(month):d>=from&&d<=to}),periodLabel=mode==="week"?week:mode==="month"?monthLabel(month):`${dateLabel(from)} – ${dateLabel(to)}`;
 const selectedWeeks=new Set(selected.map(r=>weekKey(r[14])).filter(Boolean));if(mode==="week"&&week)selectedWeeks.add(week);
 const staff=new Map<string,Staff>();for(const r of roster)staff.set(r.id,{id:r.id,name:r.name,products:{},total:fresh(),deviceTotal:fresh()});const productTotals:Record<string,Tot>=Object.fromEntries(SUPPORTED.map(k=>[k,fresh()]));
