...
export async function GET(req:NextRequest){
 ...
 try{
  const[configRows,rawDates,compiledDates]=await getSheetRanges(SHEET_ID,["Config!A1:AZ120","'RAW SalesPerson'!AB2:AB65536","'Data Copas'!A2:A50000"],email,key);
  ...
  const sourceRange=matches.length?`'RAW SalesPerson'!AB${matches[0]}:AR${matches.at(-1)}`:compiledMatches.length?`'Data Copas'!A${compiledMatches[0]}:S${compiledMatches.at(-1)}`:"",raw=sourceRange?(await getSheetRanges(SHEET_ID,[sourceRange],email,key))[0]??[]:[],rows=raw.map(parse).filter(r=>r.date===date&&valid(r));
