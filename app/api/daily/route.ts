...
const STORE="M238",SNAPSHOT="Dashboard Schedule Snapshot",TARGET_SNAPSHOT="Dashboard Daily Target Snapshot",SALES_SNAPSHOT="Dashboard Daily Sales Snapshot";
...
async function ensureMigrated(tab:string,headers:string[],range:string,appendRange:string,email:string,key:string){
 await ensureSheet(MASTER_ID,tab,headers,email,key);
 const[master]=await getSheetRanges(MASTER_ID,[range],email,key);
 if(master.some(r=>r.some(v=>s(v))))return master;
 try{const[legacy]=await getSheetRanges(SOURCE_ID,[range],email,key),rows=legacy.filter(r=>r.some(v=>s(v)));if(rows.length)await appendSheetValues(MASTER_ID,appendRange,rows,email,key);return rows}catch{return master}
}
...
async function targetForDate(date:string,configRows:unknown[][],email:string,key:string):Promise<{target:DailyTarget;locked:boolean}>{
 const existing=await ensureMigrated(TARGET_SNAPSHOT,["Date","Amount","Accessories","VAS","Created At"],`'${TARGET_SNAPSHOT}'!A2:E3000`,`'${TARGET_SNAPSHOT}'!A:E`,email,key);
 ...
 if(shouldLock)await appendSheetValues(MASTER_ID,`'${TARGET_SNAPSHOT}'!A:E`,[[date,target.amount,target.accessories,target.vas,new Date().toISOString()]],email,key);
...
async function scheduleFor(date:string,configRows:unknown[][],email:string,key:string):Promise<StaffBase[]>{
 const existing=await ensureMigrated(SNAPSHOT,["Date","Sales ID","Sales Name","Position","Share","Status","Created At"],`'${SNAPSHOT}'!A2:G3000`,`'${SNAPSHOT}'!A:G`,email,key);
 ...
 if(date<=todayJakarta()&&result.length){await appendSheetValues(MASTER_ID,`'${SNAPSHOT}'!A:G`,result.map(p=>[date,p.id,p.name,p.position,p.share,p.status,new Date().toISOString()]),email,key)}
 return result;
}
...
export async function GET(req:NextRequest){
 ...
 try{
  const[configRows,rawDates,compiledDates]=await getSheetRanges(SOURCE_ID,["Config!A1:AZ120","'RAW SalesPerson'!AB2:AB65536","'Data Copas'!A2:A50000"],email,key);
  ...
  const matches:number[]=[],compiledMatches:number[]=[];rawDates.forEach((r,i)=>{if(iso(r[0])===date)matches.push(i+2)});compiledDates.forEach((r,i)=>{if(iso(r[0])===date)compiledMatches.push(i+2)});
