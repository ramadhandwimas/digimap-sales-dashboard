...
function monthSeries(rows:unknown[][],year:number){return Array.from({length:12},(_,i)=>{const period=`${year}-${String(i+1).padStart(2,"0")}`,rr=rows.filter(r=>iso(r[0]).startsWith(period)),devices=rr.filter(r=>kind(r)==="device"),lobs:Record<string,number>={};for(const lob of DEVICE_LOBS)lobs[lob]=0;lobs["Other Device"]=0;for(const r of devices){const lob=deviceLob(r);lobs[lob]=(lobs[lob]||0)+n(r[8])}return{period,amount:sumAmount(rr),device:sumAmount(devices),lobs}})}
...
export async function GET(req:NextRequest){
 ...
 try{
  ...
  const compare=series2025.map((x,i)=>{const y=series2026[i],started=i+1<=currentMonth||y.amount>0,diff=started?y.amount-x.amount:null,growth=started&&x.amount?((y.amount-x.amount)/x.amount)*100:null,deviceDiff=started?y.device-x.device:null,deviceGrowth=started&&x.device?((y.device-x.device)/x.device)*100:null;const lobs=Object.keys({...x.lobs,...y.lobs}).map(lob=>{const amount2025=x.lobs[lob]||0,amount2026=started?(y.lobs[lob]||0):null,lobDiff=started?(amount2026||0)-amount2025:null,lobGrowth=started&&amount2025?(((amount2026||0)-amount2025)/amount2025)*100:null;return{lob,amount2025,amount2026,diff:lobDiff,growth:lobGrowth}});return{month:i+1,period2025:x.period,period2026:y.period,amount2025:x.amount,amount2026:started?y.amount:null,diff,growth,device2025:x.device,device2026:started?y.device:null,deviceDiff,deviceGrowth,lobs,started}});
  const ytdRows=compare.filter(x=>x.month<=currentMonth),ytd2025=ytdRows.reduce((a,x)=>a+x.amount2025,0),ytd2026=ytdRows.reduce((a,x)=>a+(x.amount2026||0),0),ytdGrowth=ytd2025?(ytd2026-ytd2025)/ytd2025*100:0,ytdDevice2025=ytdRows.reduce((a,x)=>a+x.device2025,0),ytdDevice2026=ytdRows.reduce((a,x)=>a+(x.device2026||0),0),ytdDeviceGrowth=ytdDevice2025?(ytdDevice2026-ytdDevice2025)/ytdDevice2025*100:0;
  const selectedMonth=Number(period.slice(5,7)),lfl=compare[selectedMonth-1]||null;
  return NextResponse.json({period,label:monthLabel(period),target,summary:{amount,device,accessories,vas,invoices:invoices.size,qty,upt:invoices.size?qty/invoices.size:0,atv:invoices.size?amount/invoices.size:0,achievement,gap,pace,status},staff,daily,team:{total:staff.length,productive:staff.filter(x=>x.status==="Productive").length,needPush:staff.filter(x=>x.status!=="Productive").length,totalTransactions:invoices.size,avgUpt:invoices.size?qty/invoices.size:0,avgAtv:invoices.size?amount/invoices.size:0},lfl,compare,ytd:{amount2025:ytd2025,amount2026:ytd2026,diff:ytd2026-ytd2025,growth:ytdGrowth,device2025:ytdDevice2025,device2026:ytdDevice2026,deviceDiff:ytdDevice2026-ytdDevice2025,deviceGrowth:ytdDeviceGrowth,throughMonth:currentMonth}},{headers:{"cache-control":"no-store"}})
