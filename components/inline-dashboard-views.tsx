...
function syncNavigation(label:string|null){
 ...
 for(const button of buttons){const current=buttonLabel(button),isItem=Object.prototype.hasOwnProperty.call(routes,current)||["Overview","Daily Sales","Staff Performance","Est. Incentive","Feedback","Settings","Data Upload"].includes(current);if(!isItem)continue;if(label&&current===label){button.classList.add("bg-white/20","shadow-sm");button.classList.remove("text-white/80");button.setAttribute("aria-current","page")}else{button.classList.remove("bg-white/20","shadow-sm");button.classList.add("text-white/80");button.removeAttribute("aria-current")}}
}
...
export default function InlineDashboardViews(){
 ...
 useEffect(()=>{
  const click=(event:MouseEvent)=>{const button=(event.target as Element|null)?.closest("button") as HTMLButtonElement|null;if(!button)return;const label=buttonLabel(button),key=routes[label];if(key){event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();syncNavigation(label);setActive({key,label});return}if(active)setActive(null)};
  document.addEventListener("click",click,true);return()=>document.removeEventListener("click",click,true);
 },[active]);

 useEffect(()=>{
  const root=document.querySelector("main.min-w-0 > div.px-4.pt-6") as HTMLElement|null;if(!root)return;let h=root.querySelector("[data-inline-view-host]") as HTMLElement|null;if(!h){h=document.createElement("div");h.dataset.inlineViewHost="1";h.className="min-w-0";root.appendChild(h)}setHost(h);
  const apply=()=>{for(const child of Array.from(root.children)){if(child===h)continue;(child as HTMLElement).style.display=active?"none":""}h!.style.display=active?"block":"none"};apply();const obs=new MutationObserver(apply);obs.observe(root,{childList:true});return()=>{obs.disconnect();for(const child of Array.from(root.children)){if(child!==h)(child as HTMLElement).style.display=""}}
 },[active]);
