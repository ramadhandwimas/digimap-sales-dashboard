...
import {SohDataActions} from "@/components/m238-data-actions";
...
export default function SohTabsPage(){
 ...
 const load=useCallback(async()=>{setLoading(true);try{const j=await(await fetch(`/api/soh?q=${encodeURIComponent(q)}&t=${Date.now()}`,{cache:"no-store"})).json();setRows(j.rows||[]);setUpdated(j.updated||"");setSoldDate(j.soldDate||"")}finally{setLoading(false)}},[q]);
 ...
 return <div className="w-full space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-blue-600 dark:text-blue-400">M238</p><h1 className="mt-1 text-3xl font-black">Stock On Hand</h1><p className="mt-1 text-slate-500">SOH per kategori{updated?` • Update stock ${updated}`:""}{soldDate?` • Qty Sold ${soldDate}`:""}.</p></div><SohDataActions onChanged={load}/></div>
