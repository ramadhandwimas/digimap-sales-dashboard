...
function getReportData(root:HTMLElement,mode:FocusMode):ReportData{
 ...
 const percent=ai>=0&&/%/.test(lines[ai+2]||"")?lines[ai+2]:"";
 const gap=pickAfter(lines,"Gap Value")||pickAfter(lines,"Gap")||"-";
 const qty=pickAfter(lines,"Actual Qty")||pickAfter(lines,"Qty")||"-";
 ...
 if(staffSection){
  for(const row of Array.from(staffSection.querySelectorAll("tbody tr"))){
   ...
   const name=cells[0]||"Staff",detail=cells.slice(1).join("  •  ");
   const active=!/^0(?:\s|$)/.test(cells[1]||"")||cells.slice(1).some(x=>/Rp(?!0(?:\D|$))/.test(x));
   staff.push({name,detail,active});
  }
 }
...
async function renderNative(data:ReportData,mode:FocusMode){
 ...
 const hasStaff=data.staff.length>0, staffHeight=hasStaff?(88+active.length*74+(zeroCount?62:0)):0;
 const height=hasStaff?Math.max(980,690+staffHeight):760;
 ...
 ctx.fillStyle="#ffffff";roundRect(ctx,28,28,width-56,height-56,32);ctx.fill();
 ...
 const cards=[{label:"Target Value",value:data.target},{label:"Achievement",value:data.achievement,sub:data.percent},{label:"Gap Value",value:data.gap},{label:"Actual Qty",value:data.qty}];
 const cw=(inner-76)/2,ch=92;
 cards.forEach((c,i)=>{const col=i%2,row=Math.floor(i/2),x=pad+26+col*(cw+24),cy=y+108+row*(ch+18);ctx.fillStyle="#ffffff";roundRect(ctx,x,cy,cw,ch,18);ctx.fill();ctx.fillStyle="#94a3b8";ctx.font="600 18px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText(c.label,x+18,cy+28);ctx.fillStyle="#0f172a";ctx.font="800 27px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText(c.value||"-",x+18,cy+62);if(c.sub){ctx.fillStyle="#64748b";ctx.font="700 18px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText(c.sub,x+18,cy+84)}});
 ...
 if(hasStaff){ctx.fillStyle="#0f172a";ctx.font="800 28px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText(`Penjualan Staff • ${data.item}`,pad,y);y+=38;ctx.fillStyle="#64748b";ctx.font="500 18px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText("Staff dengan transaksi ditampilkan lebih dulu",pad,y);y+=30;
  if(active.length===0){ctx.fillStyle="#f8fafc";roundRect(ctx,pad,y,inner,58,14);ctx.fill();ctx.fillStyle="#64748b";ctx.font="700 20px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText("Belum ada staff dengan transaksi pada pilihan ini",pad+18,y+37);y+=70}else{
   for(const row of active){ctx.fillStyle="#f8fafc";roundRect(ctx,pad,y,inner,62,14);ctx.fill();ctx.fillStyle="#0f172a";ctx.font="800 21px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText(row.name.slice(0,34),pad+18,y+26);ctx.fillStyle="#475569";ctx.font="600 18px -apple-system,BlinkMacSystemFont,Arial,sans-serif";ctx.fillText(row.detail.slice(0,72),pad+18,y+49);y+=72}
  }
