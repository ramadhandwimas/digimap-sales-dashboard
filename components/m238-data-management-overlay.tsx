...
export default function M238DataManagementOverlay(){
 ...
 useEffect(()=>{
  const sync=()=>{
   for(const button of Array.from(document.querySelectorAll("aside nav button"))){const label=button.textContent?.trim();if(label==="Data Upload")(button as HTMLElement).style.display="none"}
   ...
   if(!slot){slot=document.createElement("div");slot.dataset.m238DataActions="1";slot.className="shrink-0";actions.prepend(slot)}
   setHost(slot);
  };
 ...
 const refresh=()=>{const heading=Array.from(document.querySelectorAll("h1")).find(x=>x.textContent?.trim()==="Daily Sales");const header=heading?.parentElement?.parentElement;const btn=header?.querySelector('button[title="Perbarui data"]') as HTMLButtonElement|null;btn?.click()};
