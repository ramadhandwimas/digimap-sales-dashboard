...
type SavedSnapshot={data:Weekly;trafficA:number;trafficB:number;savedAt:string};
...
export default function WeeklyReasonPage(){
 ...
 const build=useCallback(async(week:string,force=false,knownWeeks?:string[])=>{
  ...
  try{
   if(!force){const saved=await fetch(`/api/weekly-reason-snapshot?week=${encodeURIComponent(week)}`,{cache:"no-store"}).then(r=>r.json());if(saved.found&&saved.snapshot){setSnapshot(saved.snapshot as SavedSnapshot);setSource("saved");return}}
   ...
   const [ta,tb]=await Promise.all([fetch(`/api/traffic?from=${w.periodA.start}&to=${w.periodA.end}`,{cache:"no-store"}).then(r=>r.json()),fetch(`/api/traffic?from=${w.periodB.start}&to=${w.periodB.end}`,{cache:"no-store"}).then(r=>r.json())]);
   const next:SavedSnapshot={data:w,trafficA:Number(ta.total||0),trafficB:Number(tb.total||0),savedAt:new Date().toISOString()};
 ...
 const data=snapshot?.data,trafficA=snapshot?.trafficA||0,trafficB=snapshot?.trafficB||0,salesA=total(data?.a?.scheme),salesB=total(data?.b?.scheme),salesGrowth=salesA.amount?(salesB.amount-salesA.amount)/salesA.amount*100:0,trafficGrowth=trafficA?(trafficB-trafficA)/trafficA*100:0;
 return <div className="space-y-5">
  <div><p className="text-xs font-bold uppercase tracking-[.16em] text-blue-600">M238 Reporting</p><h1 className="mt-1 text-3xl font-black">Weekly Reason</h1><p className="mt-1 text-sm text-slate-500">Reason mingguan berdasarkan Sales Summary Weekly Report, compare week-to-week, traffic, dan compile feedback team.</p></div>
  ...
  {data&&<>
   ...
   <section className="rounded-2xl border bg-white p-5 shadow-sm dark:bg-slate-950"><h3 className="font-black">Traffic</h3><div className="mt-3 grid grid-cols-2 gap-4"><div><b>{data.labelA}</b><p>{num.format(trafficA)}</p></div><div><b>{data.labelB}</b><p>{num.format(trafficB)}</p></div></div><p className={`mt-2 font-black ${trafficGrowth>=0?"text-emerald-600":"text-rose-600"}`}>Growth {trafficGrowth>=0?"+":""}{pct(trafficGrowth)}</p></section>
