...
export default function WeeklyReasonPage(){
 ...
 const reportText=()=>{
  ...
  const lines=[
   ...
   `*${data.labelB}*`,
   "",
   `Total Sales: ${money.format(salesB.amount)} (${salesGrowth>=0?"naik":"turun"} ${pct(Math.abs(salesGrowth))} vs ${data.labelA} ${money.format(salesA.amount)})`,
   "",
   `Traffic ${data.labelA}: ${num.format(trafficA)}`,
   `Traffic ${data.labelB}: ${num.format(trafficB)} (${trafficGrowth>=0?"+":""}${pct(trafficGrowth)})`,
   "",
   `Conversion ${data.labelA}: ${num.format(dailyA.invoices)} transaksi | CVR ${pct(cvrA)} | UPT ${uptA.toFixed(1)}`,
   `Conversion ${data.labelB}: ${num.format(dailyB.invoices)} transaksi | CVR ${pct(cvrB)} | UPT ${uptB.toFixed(1)}`,
   "",
   `*Compile Reason Staff*`,
   data.feedbackSummary||"Belum ada feedback team.",
  ];
  for(const [key,label] of lobOrder){const a=total(data.a?.lob?.[key]),b=total(data.b?.lob?.[key]),g=a.qty?(b.qty-a.qty)/a.qty*100:0,an=data.analysis?.[key],plan=humanPlan(key,an?.review||"",data.feedbackSummary||"");lines.push("",`*${label}*`,`${label} ${data.labelB} ${g>=0?"naik":"turun"} ${g>=0?"+":""}${pct(g)} dengan total ${num.format(b.qty)} unit, dibanding ${data.labelA} ${num.format(a.qty)} unit.`,`Reason: ${an?.review||"Belum ada reason yang tersimpan untuk LOB ini."}`,an?.target?`Achievement vs Target: ${an.gap>=0?"Plus":"Minus"} ${Math.abs(an.gap)} unit | Achievement ${pct(an.achievement)}`:"Achievement vs Target: Target belum tersedia",`Action Plan: ${plan}`)}
  return lines.join("\n");
 };
