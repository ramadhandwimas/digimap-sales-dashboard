...
function cleanReason(text:string){
 return String(text||"")
  ...
  .replace(/berdasarkan\s+stok\s+saat\s+ini[:,]?\s*/gi,"")
  .replace(/berdasarkan\s+data\s+stok[:,]?\s*/gi,"")
  .replace(/sistem\s+stok/gi,"ketersediaan stok")
  .replace(/\s+/g," ")
  .trim();
}
...
export default function WeeklyReasonPage(){
 ...
 const[weeks,setWeeks]=useState<string[]>([]),[selected,setSelected]=useState(""),[snapshot,setSnapshot]=useState<SavedSnapshot|null>(null),[loading,setLoading]=useState(true),[source,setSource]=useState<"saved"|"generated"|"">(""),[error,setError]=useState(""),[shareBusy,setShareBusy]=useState("");
 ...
 const reportText=()=>{
  ...
  const lines=[
   ...
   "",
   `*TOTAL SALES*`,
   `${money.format(salesB.amount)}`,
   `${salesGrowth>=0?"Naik":"Turun"} ${pct(Math.abs(salesGrowth))} vs ${data.labelA} (${money.format(salesA.amount)})`,
   "",
   `*TRAFFIC & CONVERSION*`,
   `${data.labelA}: Traffic ${num.format(trafficA)} | Trx ${num.format(dailyA.invoices)} | CVR ${pct(cvrA)} | UPT ${uptA.toFixed(1)}`,
   `${data.labelB}: Traffic ${num.format(trafficB)} | Trx ${num.format(dailyB.invoices)} | CVR ${pct(cvrB)} | UPT ${uptB.toFixed(1)}`,
  ];
  if(data.feedbackSummary){lines.push("",`*KONDISI FLOOR / FEEDBACK TEAM*`,cleanReason(data.feedbackSummary));}
  for(const [key,label] of lobOrder){
   const a=total(data.a?.lob?.[key]),b=total(data.b?.lob?.[key]),g=a.qty?(b.qty-a.qty)/a.qty*100:0,an=data.analysis?.[key],reason=cleanReason(an?.review||"Belum ada reason untuk LOB ini."),plan=humanPlan(key,reason,data.feedbackSummary||"");
   lines.push("",`*${label.toUpperCase()}*`,`${g>=0?"Naik":"Turun"} ${pct(Math.abs(g))} | ${num.format(a.qty)} → ${num.format(b.qty)} unit`,`Reason: ${reason}`,an?.target?`Target: ${an.gap>=0?"Plus":"Minus"} ${Math.abs(an.gap)} unit | Achievement ${pct(an.achievement)}`:"Target: belum tersedia",`Plan: ${plan}`);
  }
  lines.push("",`_M238 PIM 2 Weekly Review_`);
  return lines.join("\n");
 };
 ...
 return <div className="space-y-5">
  <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-blue-600">M238 Reporting</p><h1 className="mt-1 text-3xl font-black">Weekly Reason</h1><p className="mt-1 text-sm text-slate-500">Weekly review yang mengikuti data compare, kondisi floor, feedback team, dan action plan store.</p></div>{data&&<div className="export-hide flex flex-wrap gap-2"><button type="button" onClick={()=>void downloadPdf()} disabled={Boolean(shareBusy)} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-60">{shareBusy==="PDF"?"Membuat PDF...":"Unduh PDF"}</button><button type="button" onClick={()=>void shareText()} disabled={Boolean(shareBusy)} className="rounded-xl border bg-white px-4 py-2.5 text-sm font-black text-slate-800 shadow-sm disabled:opacity-60 dark:bg-slate-950 dark:text-white">{shareBusy==="TEXT"?"Menyiapkan...":"Share Text"}</button><button type="button" onClick={shareWhatsapp} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-black text-white">Share WhatsApp</button></div>}</div>
  ...
  {data&&<div ref={reportRef} className="space-y-5 bg-slate-50 p-1 dark:bg-slate-900"><section className="rounded-2xl border bg-white p-5 shadow-sm dark:bg-slate-950"><h2 className="text-xl font-black">M238 DIGIMAP PIM 2</h2><h3 className="mt-1 text-lg font-black">{data.labelB}</h3><p className="mt-4 leading-7">Total sales {data.labelB} <b>{money.format(salesB.amount)}</b>, {salesGrowth>=0?"naik":"turun"} <b>{pct(Math.abs(salesGrowth))}</b> dibanding {data.labelA} sebesar <b>{money.format(salesA.amount)}</b>.</p></section>
   ...
   <section className="rounded-2xl border bg-blue-50/60 p-5 dark:bg-blue-950/20"><h3 className="font-black">Compile Reason Staff</h3><p className="mt-2 leading-7">{cleanReason(data.feedbackSummary)}</p></section>
   {lobOrder.map(([key,label])=>{const a=total(data.a?.lob?.[key]),b=total(data.b?.lob?.[key]),g=a.qty?(b.qty-a.qty)/a.qty*100:0,an=data.analysis?.[key],reason=cleanReason(an?.review||"Belum ada reason yang tersimpan untuk LOB ini."),plan=humanPlan(key,reason,data.feedbackSummary||"");return <section key={key} className="rounded-2xl border bg-white p-5 shadow-sm dark:bg-slate-950"><h3 className="text-lg font-black">{label}</h3><p className="mt-3">{label} {data.labelB} {g>=0?"naik":"turun"} <b>{g>=0?"+":""}{pct(g)}</b> dengan total <b>{num.format(b.qty)} unit</b>, dibanding {data.labelA} <b>{num.format(a.qty)} unit</b>.</p><div className="mt-4"><b>Reason</b><p className="mt-1 leading-7">{reason}</p></div><div className="mt-4"><b>Achievement vs Target</b><p className={an?.gap>=0?"text-emerald-600":"text-rose-600"}>{an?.target?`${an.gap>=0?"Plus":"Minus"} ${Math.abs(an.gap)} unit • Achievement ${pct(an.achievement)}`:"Target belum tersedia"}</p></div><div className="mt-4"><b>Action Plan</b><p className="mt-1 leading-7">{plan}</p></div></section>})}</div>}
 </div>
}