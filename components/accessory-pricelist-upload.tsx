...
type Result={total:number;newCount:number;accessoryCount:number;vasCount:number;existing:number;duplicates:number;ignored:number;reviewCount:number;preview:MasterRow[];review:ReviewItem[];planId:string;imported?:number;message?:string};
...
export default function AccessoryPricelistUpload(){
 ...
 return <section className="acc-pricelist" aria-label="Upload Pricelist Accessories dan VAS">
  <div className="acc-heading"><FileSpreadsheet size={22}/><div><h3>Upload Pricelist Accessories &amp; VAS</h3><p>Tambahkan Accessories serta VAS Qoala, Telkomsel, XL, dan Indosat ke Master.</p></div></div>
  ...
  <div className="acc-actions">
   ...
   {result&&result.newCount>0&&result.imported===undefined?<button type="button" className="acc-primary" disabled={!!busy} onClick={()=>void run("import")}>
    {busy==="import"?<LoaderCircle className="animate-spin" size={17}/>:<Upload size={17}/>}{busy==="import"?"Menyimpan…":`Tambahkan ${result.newCount.toLocaleString("id-ID")} Produk`}
   </button>:null}
  </div>
  <p className="acc-note">Pengecekan memakai SAP Article dan isi Master terbaru. AppleCare tetap Accessories; Qoala, Telkomsel, XL, dan Indosat masuk VAS. Produk yang belum cocok akan ditampilkan untuk diperiksa.</p>
  ...
  {result?<div aria-live="polite" className="acc-results">
   ...
   <div className="acc-counts">
    <div><strong>{result.imported??result.newCount}</strong><span>{result.imported!==undefined?"Produk ditambahkan":"Produk siap ditambahkan"}</span></div>
   ...
   <p className="acc-note">{result.accessoryCount} Accessories · {result.vasCount} VAS · {result.total.toLocaleString("id-ID")} baris produk · {result.duplicates} duplikat dalam file · {result.ignored} baris kosong/header dilewati.</p>
   {result.newCount===0&&result.imported===undefined?<p>Tidak ada produk Accessories/VAS baru yang siap ditambahkan.</p>:null}
