...
function MoreScreen({dark,toggleDark}:{dark:boolean;toggleDark:()=>void}){const groups=[["Performance",[[WalletCards,"Incentive"],[CreditCard,"BNPL"],[Target,"Target & Program"]]],["Operational",[[ClipboardCheck,"Checklist Store"],[Activity,"NPS / CX & Member"],[TrendingUp,"Aktivitas Toko"]]],["Appearance",[[dark?Sun:Moon,dark?"Light Mode":"Dark Mode"]]],["Account",[[Settings,"Settings"],[LogOut,"Logout"]]]] as const;return <div className="m238m-more">{groups.map(([title,items])=><section key={title}><h3>{title}</h3><div>{items.map(([Icon,label])=><button key={label} onClick={()=>{if(label.includes("Mode"))toggleDark();if(label==="Logout")void fetch("/api/auth/logout",{method:"POST"}).finally(()=>{window.location.href="/login"})}}><span><i><Icon size={18}/></i>{label}</span><ChevronRight size={17}/></button>)}</div></section>)}</div>}
