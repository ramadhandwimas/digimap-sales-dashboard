...
export default function MobileDashboardApp(){
  ...
  return <div ref={rootRef} className="m238m-app" onTouchStart={e=>{if(window.scrollY===0)touchStart.current=e.touches[0].clientY}} onTouchMove={touchMove} onTouchEnd={()=>{touchStart.current=null}}>
    ...
    <nav className="m238m-bottom" style={{"--m238m-active-index":String(["home","sales","team","report","more"].indexOf(tab==="admin"?"more":tab))} as CSSProperties}>
      ...
      ].map(([key,label,Icon])=><button key={String(key)} onClick={()=>setTab(key as Tab)} className={(tab===key||(tab==="admin"&&key==="more"))?"active":""}><span className="m238m-nav-icon"><Icon size={21}/></span><span className="m238m-nav-label">{String(label)}</span></button>)}
    </nav>
...
function TeamScreen({rows,filter,setFilter,onStaff}:{rows:Staff[];filter:string;setFilter:(v:string)=>void;onStaff:(s:Staff)=>void}){
 ...
 return <div className="m238m-stack m238m-enter">
  <div className="m238m-chips">{[["all","All"],["top","Top Performance"],["low","Low Performance"]].map(([k,l])=><button key={k} className={filter===k?"active":""} onClick={()=>setFilter(k)}>{l}</button>)}</div>
  <Card className="m238m-team-disclaimer"><strong>Catatan Ranking Staff</strong><p>Penjualan dari digimap.co.id / channel online tidak termasuk dalam ranking staff store.</p></Card>
  <div className="m238m-section-head"><h2>Staff Performance</h2><span>{label}</span></div>
  <div className="m238m-list">{rows.map(s=><StaffRow key={s.id} staff={s} onClick={()=>onStaff(s)} fullMoney/>)}</div>
 </div>
}
...
function WeeklyView({weekly,summary}:{weekly:Weekly;summary:DailySummary|null}){
 ...
 return <>
  ...
  <Card className="m238m-copy-card"><strong>Weekly Detail</strong><p>Tap card Weekly Sales di atas untuk membuka compare ${weekly.labelA} vs ${weekly.labelB}: Summary, LOB, VAS, dan Reason.</p></Card>
