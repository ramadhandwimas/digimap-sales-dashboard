...
function FeedbackView({data}:{data:Feedback;staff:Staff[]}){
 const[selectedDate,setSelectedDate]=useState(today()),[rows,setRows]=useState(data.rows.filter(r=>r.date===today())),[dailyStaff,setDailyStaff]=useState<Staff[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState("");
 const load=useCallback(async(date:string)=>{
  ...
  try{
   ...
   setRows(fd.rows||[]);setDailyStaff(dd.staff||[]);
...
function MoreScreen({dark,toggleDark,onAction}:{dark:boolean;toggleDark:()=>void;onAction:(action:string)=>void}){
 const groups=[
  ["Performance",[[MessageCircle,"Tambah Feedback","add-feedback"],[WalletCards,"Incentive","incentive"],[Users,"NPS / CX & Member","cx"],[TrendingUp,"Aktivitas Toko","activity"]]],
