 ...
 return <>
  ...
  <Sheet open={showDetail} onClose={()=>setShowDetail(false)} title={`Weekly Compare • ${weekly.labelA} vs ${weekly.labelB}`}>
   <div className="m238m-stack">
    ...
    <div className="m238m-section-head"><h2>LOB Performance</h2><span>Target vs Actual</span></div>
    <div className="m238m-list">{lobs.map(([name,key])=>{const a=sum(weekly.a.lob?.[key]||{}),b=sum(weekly.b.lob?.[key]||{}),target=weekly.targets?.lob?.[key]||0,ach=target?b.qty/target*100:0,d=delta(a.qty,b.qty);return <Card key={key} className="m238m-week-compare-row"><div className="m238m-week-compare-head"><strong>{name}</strong><b className={d>=0?"positive":"negative"}>{d>=0?"+":""}{pct(d)} Qty</b></div><div className="m238m-week-compare-values"><div><span>{weekly.labelA}</span><b>{num.format(a.qty)} unit</b><small>{money.format(a.amount)}</small></div><div><span>{weekly.labelB}</span><b>{num.format(b.qty)} unit</b><small>{money.format(b.amount)}</small></div></div>{target?<div className="m238m-week-target"><div><span>Target {num.format(target)}</span><b>{pct(ach)}</b></div><Progress value={ach}/><small>Gap {b.qty-target>=0?"+":""}{num.format(b.qty-target)} unit</small></div>:null}</Card>})}</div>
