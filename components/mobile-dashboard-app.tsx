...
type SheetName="period"|"share"|"staff"|null;
...
export default function MobileDashboardApp(){
  const[tab,setTab]=useState<Tab>("home"),[period,setPeriod]=useState(periodNow()),[draftPeriod,setDraftPeriod]=useState(periodNow()),[sheet,setSheet]=useState<SheetName>(null);
  ...
  return <div ref={rootRef} className="m238m-app" onTouchStart={e=>{if(window.scrollY===0)touchStart.current=e.touches[0].clientY}} onTouchMove={touchMove} onTouchEnd={()=>{touchStart.current=null}}>
    ...
    <main className="m238m-content">
      ...
      {tab==="report"?<ReportScreen mode={reportMode} setMode={setReportMode} weekly={weekly} weeklySummary={weeklySummary} feedback={feedback} cx={cx}/>:null}
      {tab==="more"?<MoreScreen dark={dark} toggleDark={toggleDark}/>:null}
    </main>
...
function StaffDetail({staff}:{staff:Staff}){const target=staff.targets?.amount||staff.target||0,a=target?staff.amount/target*100:staff.achievement||0;return <div className="m238m-stack"><div className="m238m-profile"><div className="m238m-avatar big">{initials(staff.name)}</div><div><h2>{staff.name}</h2><p>{staff.position||"Staff M238"}</p></div></div><Card className="m238m-hero compact"><span>Sales</span><strong>{compact(staff.amount)}</strong><p>Target {compact(target)} • {pct(a)}</p><Progress value={a}/></Card><div className="m238m-grid"><Metric label="Device" value={compact(staff.device)}/><Metric label="ACC" value={compact(staff.accessories)}/><Metric label="VAS" value={compact(staff.vas)}/><Metric label="UPT" value={staff.upt.toFixed(1)}/><Metric label="ATV" value={compact(staff.atv)}/><Metric label="Invoice" value={num.format(staff.invoices)}/><Metric label="Qoala" value={compact(staff.vasDetail?.qoala?.value||0)}/><Metric label="Incentive" value={compact(staff.incentive?.total||0)}/></div></div>}

function ReportScreen({mode,setMode,weekly,weeklySummary,feedback,cx}:{mode:ReportMode;setMode:(v:ReportMode)=>void;weekly:Weekly|null;weeklySummary:DailySummary|null;feedback:Feedback|null;cx:Cx|null}){
 return <div className="m238m-stack m238m-enter"><Segmented value={mode} onChange={setMode} items={[{value:"weekly",label:"Weekly"},{value:"feedback",label:"Feedback"},{value:"cx",label:"CX"}]}/>{mode==="weekly"?(weekly?<WeeklyView weekly={weekly} summary={weeklySummary}/>:<Skeleton/>):mode==="feedback"?(feedback?<FeedbackView data={feedback}/>:<Skeleton/>):(cx?<CxView data={cx}/>:<Skeleton/>)}</div>
}
...
function FeedbackView({data}:{data:Feedback}){return <><div className="m238m-section-head"><h2>Feedback Team</h2><span>{data.rows.length} data</span></div><div className="m238m-list">{data.rows.length?data.rows.slice().reverse().map((r,i)=><Card key={`${r.date}-${r.staffId}-${i}`} className="m238m-copy-card"><div className="m238m-copy-head"><strong>{r.name}</strong><span>{r.date}</span></div><p>{r.professional||r.raw}</p></Card>):<Card>Belum ada feedback pada periode ini.</Card>}</div></>}
function CxView({data}:{data:Cx}){const cx=data.rows.reduce((a,r)=>a+r.cx,0),member=data.rows.reduce((a,r)=>a+r.member,0);return <><div className="m238m-grid"><Metric label="CX" value={num.format(cx)}/><Metric label="New Member" value={num.format(member)}/></div><div className="m238m-list">{data.rows.slice().reverse().map((r,i)=><Card key={`${r.date}-${r.staffId}-${i}`} className="m238m-copy-card"><div className="m238m-copy-head"><strong>{r.name}</strong><span>{r.date}</span></div><p>CX {num.format(r.cx)} • New Member {num.format(r.member)}</p></Card>)}</div></>}

function MoreScreen({dark,toggleDark}:{dark:boolean;toggleDark:()=>void}){const groups=[["Performance",[[WalletCards,"Incentive"],[CreditCard,"BNPL"],[Target,"Target & Program"]]],["Operational",[[ClipboardCheck,"Checklist Store"],[Activity,"NPS / CX & Member"],[TrendingUp,"Aktivitas Toko"]]],["Appearance",[[dark?Sun:Moon,dark?"Light Mode":"Dark Mode"]]],["Account",[[Settings,"Settings"],[LogOut,"Logout"]]]] as const;return <div className="m238m-more">{groups.map(([title,items])=><section key={title}><h3>{title}</h3><div>{items.map(([Icon,label])=><button key={label} onClick={()=>{if(label.includes("Mode"))toggleDark();if(label==="Logout")window.location.href="/api/auth/logout"}}><span><i><Icon size={18}/></i>{label}</span><ChevronRight size={17}/></button>)}</div></section>)}</div>}
