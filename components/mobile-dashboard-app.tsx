...
function YtdOverview({overview}:{overview:Overview}){
 ...
 return <>
  ...
  {lobs.length?<><div className="m238m-section-head"><h2>Top LOB YTD</h2><span>2026 vs 2025</span></div><div className="m238m-list">{lobs.map(r=><Card key={r.lob} className="m238m-compare-row"><div><strong>{r.lob}</strong><span>{num.format(r.qty2026||0)} vs {num.format(r.qty2025)} unit</span></div><div><b>{money.format(r.amount2026||0)}</b><small className={(r.growth||0)>=0?"positive":"negative"}>{(r.growth||0)>=0?"+":""}{pct(r.growth||0)}</small></div></Card>)}</div></>:null}
 </>
}
