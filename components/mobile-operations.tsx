...
import {cachedJson,peekJsonCache} from "@/lib/m238-client-cache";
...
function SohMobile(){
 ...
 useEffect(()=>{let alive=true;const url="/api/soh?q="+encodeURIComponent(q),hit=peekJsonCache<any>(url);if(hit){setData(hit.data);setLoading(false)}const t=setTimeout(async()=>{if(!hit)setLoading(true);try{const j=await cachedJson<any>(url,180000,false,{scope:"soh-search"});if(alive){setData(j);setError("")}}catch(e){if(alive)setError(data?"Gagal memperbarui SOH. Menampilkan data terakhir.":e instanceof Error?e.message:"Gagal membaca SOH")}finally{if(alive)setLoading(false)}},180);return()=>{alive=false;clearTimeout(t)}},[q]);
...
function MadingMobile({period,periodMode,selectedWeek,activeRange}:{period:string;periodMode:"month"|"week";selectedWeek:string;activeRange:{from:string;to:string}|null}){
 ...
 useEffect(()=>{let alive=true;const weekly=periodMode==="week"&&activeRange,mUrl=weekly?`/api/mading?from=${activeRange!.from}&to=${activeRange!.to}`:`/api/mading?period=${period}`,oUrl=weekly?`/api/overview?period=${activeRange!.from.slice(0,7)}&from=${activeRange!.from}&to=${activeRange!.to}&label=${encodeURIComponent(selectedWeek)}`:`/api/overview?period=${period}`,mh=peekJsonCache<any>(mUrl),oh=peekJsonCache<any>(oUrl);if(mh)setData(mh.data);if(oh)setOverview(oh.data);setLoading(!(mh&&oh));Promise.all([cachedJson<any>(mUrl,180000,false,{scope:"mading"}),cachedJson<any>(oUrl,180000,false,{scope:"mading-overview"})]).then(([m,o])=>{if(!alive)return;setData(m);setOverview(o);setError("")}).catch(e=>alive&&setError((mh||oh)?"Gagal memperbarui data. Menampilkan data terakhir.":e instanceof Error?e.message:"Gagal membaca Mading")).finally(()=>alive&&setLoading(false));return()=>{alive=false}},[period,periodMode,selectedWeek,activeRange]);
...
function BnplMobile({period,periodMode,selectedWeek,activeRange}:{period:string;periodMode:"month"|"week";selectedWeek:string;activeRange:{from:string;to:string}|null}){
 ...
 const load=async(force=false)=>{const periods=periodMode==="week"&&activeRange?[...new Set([activeRange.from.slice(0,7),activeRange.to.slice(0,7)])]:[period],urls=periods.map(p=>"/api/bnpl?period="+p),cached=urls.map(u=>peekJsonCache<any>(u)).filter(Boolean);if(cached.length===urls.length){const parts=cached.map(x=>x!.data),base=parts[0]||{},rows=parts.flatMap(x=>x.rows||[]);setData({...base,rows});setLoading(false)}else setLoading(!data);try{const responses=await Promise.all(urls.map((u,i)=>cachedJson<any>(u,180000,force,{scope:"bnpl-"+i}))),base=responses[0]||{},rows=responses.flatMap(x=>x.rows||[]);setData({...base,rows});setError("")}catch(e){setError(data?"Gagal memperbarui data. Menampilkan data terakhir.":e instanceof Error?e.message:"Gagal membaca BNPL")}finally{setLoading(false)}};
