...
export default function DailySummaryLobFocus({mode,period,from,to}:{mode:Mode;period:string;from:string;to:string}){
 const[data,setData]=useState<FocusPayload|null>(null),[targets,setTargets]=useState<TargetMap>({}),[draft,setDraft]=useState<TargetMap>({}),[category,setCategory]=useState<"Semua"|"iPhone"|"MacBook"|"iPad"|"Apple Watch">("Semua"),[saving,setSaving]=useState(false),[message,setMessage]=useState("");
 ...
 const shown=useMemo(()=>category==="Semua"?all:(catalog?.[category]??[]),[category,all,catalog]);
 ...
 return <section className="rounded-2xl border bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-950">
  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
   <div><h3 className="font-extrabold">Target LOB Focus</h3><p className="mt-1 text-sm text-slate-500">Sumber dan target sama dengan menu LOB Target Fokus.</p></div>
   <button onClick={save} disabled={saving||!all.length} className="rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">{saving?"Menyimpan...":"Simpan Target"}</button>
  </div>
  <div className="mt-4 flex gap-2 overflow-x-auto pb-1">{(["Semua","iPhone","MacBook","iPad","Apple Watch"] as const).map(x=><button key={x} onClick={()=>setCategory(x)} className={`whitespace-nowrap rounded-xl border px-3 py-2 text-xs font-bold ${category===x?"border-blue-600 bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300":"bg-white text-slate-500 dark:bg-slate-900"}`}>{x}</button>)}</div>
