...
async function makeLobPicture(data:DailyPayload){
 const rows=data.staff||[],base=baseReportCanvas("LOB Apple Daily","iPhone, MacBook, iPad, dan Watch = device Apple • AirPods = accessories Apple",rows.length);
...
async function makeVasPicture(data:DailyPayload){
 const rows=data.staff||[],base=baseReportCanvas("VAS Daily","Sumber RAW SalesPerson AB–AR",rows.length);
...
async function makeShareFiles(data:DailyPayload){
 ...
 const [daily,lob,vas]=await Promise.all([makeDailySalesPicture(data),makeLobPicture(data),makeVasPicture(data)]);
 return{
  stamp,
  files:[
...
export default function DailySalesAlerts(){
 useEffect(()=>{
  ...
  const inject=()=>{const h=Array.from(document.querySelectorAll("h1")).find(x=>(x.textContent||"").trim()==="Daily Sales") as HTMLElement|undefined;if(!h)return;const root=h.closest("div.space-y-5") as HTMLElement|null;if(!root||root.querySelector("[data-daily-share]"))return;const box=document.createElement("div");box.dataset.dailyShare="1";box.className="export-hide rounded-2xl border bg-white p-4 shadow-sm dark:bg-slate-950";box.innerHTML='<div class="flex flex-wrap items-center justify-between gap-3"><div><p class="font-black">Share Report</p><p class="mt-1 text-xs text-slate-500">3 gambar sekaligus • Daily Sales Staff • LOB Apple • VAS Daily</p></div><div class="flex flex-wrap gap-2"><button data-pic class="rounded-xl border px-4 py-2 text-sm font-black">Buat 3 Foto</button><button data-share class="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-black text-white">Share WhatsApp</button></div></div><p data-note class="mt-2 text-xs font-semibold text-slate-500"></p>';const header=h.parentElement?.parentElement;if(header?.nextSibling)root.insertBefore(box,header.nextSibling);else root.prepend(box);const note=box.querySelector("[data-note]") as HTMLElement;const build=async()=>{const d=await fetchDaily();return{data:d,...await makeShareFiles(d)}};(box.querySelector("[data-pic]") as HTMLButtonElement).onclick=async()=>{try{note.textContent="Membuat 3 foto report…";const{files}=await build();downloadFiles(files);note.textContent="3 foto berhasil dibuat: Daily Sales Staff, LOB Apple, dan VAS Daily."}catch(e){note.textContent=e instanceof Error?e.message:"Gagal membuat picture"}};(box.querySelector("[data-share]") as HTMLButtonElement).onclick=async()=>{try{note.textContent="Menyiapkan 3 foto untuk dibagikan…";const{files,stamp}=await build(),text=`M238 PIM 2\nUpdate sales jam ${stamp} WIB`;const multiShareSupported=!!navigator.share&&(!navigator.canShare||navigator.canShare({files}));if(multiShareSupported){await navigator.share({files,title:"M238 Daily Sales",text});note.textContent=`3 foto siap dibagikan • update ${stamp} WIB`}else{downloadFiles(files);note.textContent="Browser ini belum mendukung share 3 file sekaligus. Ketiga PNG sudah dibuat untuk dikirim manual ke WhatsApp."}}catch(e){if(!(e instanceof DOMException&&e.name==="AbortError"))note.textContent=e instanceof Error?e.message:"Gagal share"}}};
