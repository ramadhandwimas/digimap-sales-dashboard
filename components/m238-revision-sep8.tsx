...
function activePeriod(){for(const s of Array.from(document.querySelectorAll<HTMLSelectElement>("select"))){if(/^20\d{2}-\d{2}$/.test(s.value))return s.value}return today().slice(0,7)}
...
export default function M238RevisionSep8(){const[period,setPeriod]=useState(today().slice(0,7)),[host,setHost]=useState<HTMLElement|null>(null);
 useEffect(()=>{document.body.classList.add("m238-revision-stable");const sync=()=>{const p=activePeriod();setPeriod(v=>v===p?v:p);const h=Array.from(document.querySelectorAll("h2,h3")).find(x=>(x.textContent||"").includes("M238 Sales Overview")) as HTMLElement|undefined;if(!h)return;const sec=(h.closest("section")||h.parentElement?.parentElement) as HTMLElement|null;if(!sec)return;for(const child of Array.from(sec.children)){const el=child as HTMLElement;if(el.dataset.m238OverviewV2)continue;if(el.classList.contains("grid"))el.style.display="none"}let x=sec.querySelector("[data-m238-overview-v2]") as HTMLElement|null;if(!x){x=document.createElement("div");x.dataset.m238OverviewV2="1";x.className="mt-4";sec.appendChild(x)}setHost(v=>v===x?v:x)};sync();const timer=setInterval(sync,1800);const onChange=(e:Event)=>{const target=e.target as HTMLSelectElement|null;if(target?.tagName==="SELECT"&&/^20\d{2}-\d{2}$/.test(target.value)){setPeriod(target.value);setTimeout(sync,0)}};document.addEventListener("change",onChange,true);return()=>{clearInterval(timer);document.removeEventListener("change",onChange,true);document.body.classList.remove("m238-revision-stable")}},[]);
