...
export function parseSpwToNormalized(rows:unknown[][],classification:Map<string,Classification>,store="M238"){
 ...
 for(let i=0;i<rows.length;i++){
  ...
  const invoice=text(next[1]),amount=num(next[2]);sourceItems++;i++;
  if(!invoice||amount===0)continue;
  const article=up(b),description=a;if(/VOUCHER/i.test(`${article} ${description}`))continue;
  const c=classification.get(article);if(!c)unknownClassification++;
  const key=`${store}|${date}|${invoice}|${article}|${id}`;
 ...
 return{rows:[...out.values()],duplicateSkipped,unknownClassification,sourceItems};
}

function kind(r:FastSalesRow){if(r.scheme==="VAS")return"vas";if(r.scheme==="ACCESSORIES")return"accessories";if(r.scheme==="DEVICES")return"device";return"other"}
function product(r:FastSalesRow){if(r.brand!=="APPLE")return"";if(r.scheme==="ACCESSORIES"&&r.category==="AIRPODS")return"airpods";if(r.scheme!=="DEVICES")return"";if(r.category==="IPHONE")return"iphone";if(r.category==="MAC")return"mac";if(r.category==="IPAD")return"ipad";if(r.category==="APPLE WATCH")return"watch";return""}
function vasType(r:FastSalesRow){const t=`${r.article} ${r.brand} ${r.vendor} ${r.description}`.toUpperCase();if(t.includes("QOALA")||t.includes("KLA"))return"qoala";if(t.includes("TELKOMSEL")||t.includes("TSL"))return"telkomsel";if(t.includes("INDOSAT")||t.includes("IDT"))return"indosat";if(/(^|\s)XL(\s|$)|XXL/.test(t))return"xl";return""}
export function aggregateDaily(rows:FastSalesRow[],validation="PENDING_RAW_SYNC"){
 const groups=new Map<string,FastSalesRow[]>();for(const r of rows){const k=`${r.date}|${r.id}|${r.store}`;const list=groups.get(k)||[];list.push(r);groups.set(k,list)}
 ...
 for(const list of groups.values()){
  ...
  for(const r of list){amount+=r.amount;qty+=r.qty;if(kind(r)==="accessories")accessories+=r.amount;if(kind(r)==="vas")vas+=r.amount;const p=product(r);if(p==="iphone")iphone+=r.qty;else if(p==="mac")mac+=r.qty;else if(p==="ipad")ipad+=r.qty;else if(p==="watch")watch+=r.qty;else if(p==="airpods")airpods+=r.qty;const vt=vasType(r);if(vt==="qoala"){qoalaQty+=r.qty;qoalaValue+=r.amount}else if(vt==="telkomsel"){telkomselQty+=r.qty;telkomselValue+=r.amount}else if(vt==="xl"){xlQty+=r.qty;xlValue+=r.amount}else if(vt==="indosat"){indosatQty+=r.qty;indosatValue+=r.amount}}
